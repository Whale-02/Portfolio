 ...
 * @returns element
 */
export const createModelViewerElement = (src) => {
...
export function screenshot(element) {
    const renderer = getOwnPropertySymbolValue(element, 'renderer');
    if (!renderer)
...
export function AverageHSL(arr) {
    const H = [];
    const S = [];
    const L = [];
...
function maxValue(bytes) {
    return Math.pow(2, 8 * bytes) - 1;
}
function percentage(n1, n2, maxN) {
